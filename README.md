...
- 数値表現(調査結果): `number.ts` に `DIC`/`Numeral` 基盤(`jpn`(漢字/大字/rubys)、`old_jpn`(みっか/はつか等の古い日本語数え方)、`english`、`roman`、`angle`)は既に実装済みで、`jpn`/`english`/`roman` の format/parse 往復は `__tests__/fancy-date-spec.js`(「numeral dictionaries format numeric tokens」)で検証済み。だが `sample/calendars.ts` にはこれを使うサンプル暦が1つもない(`.numeral()` 自体は `fancy-date.ts` にあるが呼び出し箇所ゼロ)。さらに `old_jpn` の目玉である助数詞依存の不規則形(3日→みっか等)は `format_number()` の全呼び出し箇所(5箇所)で `appendix` 引数が渡されておらず、`format()` 経由では構造的に到達不能(`Numeral.parse()` を直接呼べば動く)。課題は主に次の3点:
  ...
  - (b) 助数詞(appendix)の配線(深掘り・設計確定): `old_jpn` の音便コールバックは `(num, tail)` のみに依存する純粋な処理で、隠れた文脈依存は無い(20 は `tail='か'` なら「はつか」、`tail='つ'`(既定)なら「はたち」と、tail だけで一意に決まる)。ただし同じトークン `d` を1つの暦定義の中で常に同じ意味(日付)で使う「99%のケース」と、同一トークンを異なる format 文字列で異なる意味(素の `d` と `d日` 付き)に使い分けたい「稀なケース」は区別が必要で、後者は今回のスコープ外として明示的に見送る。採用する仕組みは**トークンごとに固定の appendix 文字列を `algo()` に静的設定する方式**(format 文字列側のリテラル文字("日"等)を都度覗き見て推測する方式は、暦非依存であるべき `format_by()` に「日→か」のようなカレンダー固有語彙を埋め込むことになり却下)。具体的には `IndexerProps` に4要素目(`appendix: string`)を追加し(`zero` は「末尾が数値の要素」を走査して拾う形に変更し、既存の `[list, rubys, relatives]` 3要素形と衝突しないようにする)、`Indexer` に `appendix` フィールドを追加。`format_number()` の5呼び出し箇所のうち、年トークン(`y`/`Y`)を処理する3箇所(`format_by()` 内)は `dic.appendix` をその場で渡すだけで済むが、`d` 等の汎用トークンを処理する1箇所(`def_to_label()` の `integer()` クロージャ)は現在複数トークン(`H,N,m,s,S,Y,u,y` や `D,Q,d,p,w` 等)で1つのクロージャを共有しているため、トークンごとに `() => indexer.appendix` を束縛し直す配線変更が必要。設定側は `.algo({ d: [null, null, undefined, 'か'] }).numeral(old_jpn.rubys)` のように第4要素へ助数詞を書くだけになる見込み。
  - (c) 逆引き parse map(`ensure_number_map()`)は `appendix=''` 固定で構築されるため、(b)を配線しても「はつか」等の逆引きはまだ失敗する(既存のギャップは変わらず残る)。ただし `appendix` が `Indexer` 上の第一級・検査可能なプロパティになるため、将来 `ensure_number_map()` を appendix ごとに複数マップを持つ形へ拡張する際の設計コストは下がる。(b)が固まった今回のスコープでも、(c)自体はまだ着手しない。
