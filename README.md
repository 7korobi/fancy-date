...
- solar_terms_mean など mean モデル経路の SolarObservation 補完: `hasSolarEvents` を持たない簡易(mean)太陽モデル経由の `solor()`(`phenomena/solar.ts` の `solor()` 関数、Gregorian など多くの標準暦が使う経路)は、`南中高度`/`has_sunrise`/`is_up_all_day` を今回追加したが、`日の出方位`/`日の入方位` は依然未実装(実測: 該当フィールドがオブジェクトに存在しない)。方位角の導出精度を別途検証してから追加する。
- 極域現象での不定時法(`SolarDayHourTempoRule`)の扱い: 白夜・極夜で日の出/日の入が NaN になる場合、これに依存する不定時法(江戸時代の時刻system、`tempo.ts`/`phenomena/solar.ts` が日の出/日の入を直接算術に使っている)の時刻計算がどう振る舞うかは未検証。`is_up_all_day` を使えば白夜/極夜そのものは判定できるようになったが、不定時法の「1時(とき)の長さ」計算自体を極域でどう定義すべきかは別途検討が要る。
