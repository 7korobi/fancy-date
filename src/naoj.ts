...
function signed_degree_diff(a: number, b: number) {
  return mod(a - b + 180, 360) - 180
}

function julian_day(utc: number) {
  return utc / MSEC_PER_DAY + 2440587.5
}

function utc_year(utc: number) {
...
function delta_t_sec(utc: number) {
  const year = utc_year(utc)
  const t = year - 2000
  return 62.92 + 0.32217 * t + 0.005589 * t * t
}

function sin_deg(deg: number) {
  return Math.sin(deg * DEG_TO_RAD)
}

function cos_deg(deg: number) {
