...
const アマンタ = new FancyDate((c) =>
  baseCalendar(c)
    .spot(...Madurai)
    .era('サカ歴', '紀元前')
    .calendar(['1891-11-25(木) 庚戌-辛巳 三碧木-七赤金', 'y-M-d(dC7) yC60-dC60 yC9-dC9', 0])
    .division({ H: 'solar' })
...
const タイ太陰太陽暦公式 = new FancyDate(アマンタ, (c) =>
  c
    .spot(...Bangkok)
    .era('พ.ศ.', 'ก่อน พ.ศ.')
    .calendar(['2513-1-24', 'y-M-d', 0])
    .thaiOfficialLunisolar()
    .notation({
      M: [タイ太陰月名, タイ太陰月名RTGS],
    }),
)

// 公式年表そのものではなく、既知のタイ近代規則を計算するモデル。
// 旧名は互換性のため Calendar export に残し、新しい意味付けでは
// 「タイ近代太陰太陽暦」として扱う。
const タイ近代太陰太陽暦 = タイ太陰太陽暦公式
