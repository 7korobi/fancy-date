...
季節現象・暦注は、天体イベントを直接返す関数群として増やさず、`CalendarNotePolicy`の実装へ集約する。`SolarTermPolicy`はmean／observedのterm setを返し、`ZassetsuPolicy`はそのterm setから雑節を合成し、`JapaneseFixedDateNotePolicy`は節句・風習、`ReligiousFixedDateNotePolicy`はカトリック・仏教の固定日noteを返す。公開APIは`FancyDate.note()`を正本とし、term set・雑節・節句の中間結果はpolicyとnote構築内部に留める。独立した天文primitiveである`solar_phase()`は別責務として維持する。
