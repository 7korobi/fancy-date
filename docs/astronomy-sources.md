...
対象: `src/astronomy-data.ts` の `MEAN_*` 個別定数と、それらを束ねる `MEAN_ASTRONOMY`。

| データ                                   | 主な項目                                 | 出典・状態                                                                                                                                                                                                  |
| ---------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 太陽・惑星・月・主な衛星・準惑星の本体値 | 半径など                                 | 理科年表由来として扱う。ただし旧コードには版・ページが残っていないため、正確な版情報は未特定。                                                                                                              |
| 平均季節年                               | 観測者が春分を迎える平均周期・基準 epoch | 惑星は主に JPL SSD approximate positions の平均黄経変化率から換算。複数天体の epoch は春分相当の `2019-03-21 06:58 JST` (`2019-03-20T21:58:00Z`) にそろえている。                                           |
| 平均太陽日                               | 観測者が見る南中周期・赤道傾斜           | 物理自転周期をそのまま置かず、公転の影響を含めた平均太陽日として扱う。地球は暦上の1日として `86400000ms` に固定する。火星・海王星など、旧平均値からズレが大きかったものは NASA/JPL の代表値から再換算した。 |
...
| 平均太陽日・赤道傾斜 | `src/astronomy-data.ts` の `solarDay` と同系統。Mars24 の sol と揃える。 |
...
| 平均太陽日・赤道傾斜    | `src/astronomy-data.ts` の `solarDay`。物理自転周期に公転の影響を加味して平均南中周期へ換算する。                                                      |
...
| 平均太陽日・赤道傾斜 | `src/astronomy-data.ts` の `solarDay`。SBDB の平均運動と NASA/JPL 代表値から平均南中周期へ換算する。       |
...
現状は `MeanPlanetSolarOrbital` の平均黄経モデル。平均季節年は JPL SSD approximate positions の平均黄経変化率、平均太陽日は NASA/JPL の代表的な自転周期を南中周期へ換算した `solarDay` を使う。高精度化する場合は、水星・金星と同じ JPL SSD approximate positions の Table 1/2 へ移行できる。ただし巨大惑星は固体表面を持たないため、日の出・日の入は「基準回転楕円体上の観測者」というモデル化になる。

## 今後の注意

- 理科年表由来として残っていた旧平均値は、版情報が未特定。値を更新するときは、版・ページ・単位変換を必ず残す。
