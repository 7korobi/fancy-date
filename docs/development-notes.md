...
- `span_from_labels(token, from, to)` は不断cycle限定のsymbolic変換。cycle上の順方向mod差を年cycle=`y`/日cycle=`d`へ割り当て、常に非負のanchorなしSpanを返す。`R6`/`LM27`/`Q`は非連続なのでinvalid。厳密な時刻差は`find_span()`の責務。
