...
- 太陽太陰暦一般化のPhase 1として、`policy-regression-spec.js` に年/月/日/時刻区間の半開境界、階層包含、日/月送り、不定時法の単調性を追加した。Phase 2では `src/phenomena/calendar-policy.ts` に `CalendarYearPolicy`、`LunisolarPolicy`、`HourDivisionPolicy` などの型契約を追加し、公開barrelへ再exportした。まだ既存の計算経路へ接続せず、型と回帰specを先に固定する段階である。
...
- 太陽太陰暦一般化のPhase 8として、既存の`AssignmentRule`を`DayAssignmentPolicy`へadapter経由で実行するよう整理した。tithiのraw連続index、civil dayへの投影、`repeated`/`skipped` flags、cache semanticsは既存specで維持し、assignmentがday boundary policyとは別軸であることを型契約へ反映した。
- 太陽太陰暦一般化のPhase 9として、年ごとの宗教行事をcivil dateへ返す`FeastPolicy`契約を追加し、Computusの教会祝祭日を`ChurchFeastPolicy` adapterへ接続した。`church_feasts()`と`churchFeastDates()`の互換APIは維持し、Computusの伝統、表示先の市民暦、表示ラベルはそれぞれ計算policy・projection・notationの別責務として残した。
