...
Object.defineProperty(exports, 'EarthMoonOrbital', {
  enumerable: true,
  get: function () {
    return naoj_1.EarthMoonOrbital
  },
})
Object.defineProperty(exports, 'EarthSolarOrbital', {
  enumerable: true,
  get: function () {
    return naoj_1.EarthSolarOrbital
  },
})
...
Object.defineProperty(exports, 'MarsSolarOrbital', {
  enumerable: true,
  get: function () {
    return nasa_1.MarsSolarOrbital
  },
})
...
Object.defineProperty(exports, 'prepareSpot', {
  enumerable: true,
  get: function () {
    return prepare_2.prepareSpot
  },
})
Object.defineProperty(exports, 'prepareSpotModels', {
  enumerable: true,
  get: function () {
    return prepare_2.prepareSpotModels
  },
})
...
class FancyDate {
  ...
  span_parse_rows() {
    return [
      ...
      'Zz',
      'u',
    ].map((token) => [token, this.span_part_unit(token), this.span_part_fallback_unit(token)])
  }
  ...
  infer_find_step_from_format(format) {
    ...
    for (const token of tokens) {
      const candidate = this.find_step_for_token(token[0])
  ...
  span_args(from, options) {
    if (from == null) return [Date.now(), options]
    if (this.is_span_options(from)) return [Date.now(), from]
  ...
  def_zero() {
    ...
    const rawX = (0, time_1.to_tempo_bare)(
      this.calc.msec.hour,
  ...
  */
  ...
  to_tempo_by_solor(utc, day) {
    return (0, solar_1.to_tempo_by_solor)(
      this.dic.sunny,
      this.dic.earthy,
      this.dic.geo,
      this.calc.msec.day,
   ...
   */
  orbital_season_rule() {
    return (this._orbital_season_rule ??= new tempo_1.CachedTempoRule(
      new tempo_1.OrbitalPhaseTempoRule(this.dic.sunny, this.dic.Z.length, 1 / 8),
    ))
  }
  /**
   ...
   */
  solar_hour_rule() {
    return (this._solar_hour_rule ??= new tempo_1.CachedTempoRule(
      new tempo_1.SolarDayHourTempoRule(
        this.dic.sunny,
        this.dic.earthy,
        this.dic.geo,
        this.calc.msec.day,
  ...
  to_tempos(utc) {
    ...
    const Zz = tempo_1.Tempo.at(
      new tempo_1.FixedTempoRule(this.calc.msec.year, this.calc.zero.season),
      {
        write_at: utc,
      },
    ) // 太陽年
    ...
    if (this.dic.moony && moon_msec != null && !usesObservedLunisolar) {
      ...
      // される不具合があった)。
      Nn = tempo_1.Tempo.at(
        new tempo_1.MeanLunisolarMonthRule(
          moon_msec,
          this.calc.zero.moon,
          this.calc.msec.day,
          this.calc.zero.day,
      ...
      N = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.day), {
        write_at: utc,
        parent: (0, tempo_1.envelope_of)(Nn),
      })
    }
    if (this.is_table_leap) {
      p = tempo_1.Tempo.at(
        new tempo_1.FixedTempoRule(this.calc.msec.period, this.calc.zero.period),
        {
          write_at: utc,
        },
      )
      ...
      u = tempo_1.Tempo.at(
        new tempo_1.TableTempoRule(this.table.msec.year, this.calc.zero.period),
        {
          write_at: utc,
        },
      )
      M = tempo_1.Tempo.at(new tempo_1.TableTempoRule(this.table.msec.month[u.size], u.last_at), {
        write_at: utc,
      })
      d = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.day), {
        write_at: utc,
      ...
      if (this.is_table_month) {
        u = tempo_1.Tempo.at(
          new tempo_1.FloorTempoRule(this.calc.msec.year, this.calc.zero.spring, [
        ...
        if (usesObservedLunisolar) {
          ...
          // 年境界でリセットされる。
          u = tempo_1.Tempo.at(
            new tempo_1.EraAdjustedTempoRule(
              new tempo_1.ObservedLunisolarYearRule((at) => this.lunisolar(at)),
              this.calc.msec.year,
              this.table.msec.era,
              this.calc.zero.era,
              this.calc.eras,
          ...
          M = tempo_1.Tempo.at(
            new tempo_1.ObservedLunisolarMonthRule((at) => this.lunisolar(at), moon_msec),
            {
              write_at: utc,
            },
          )
    ...
    if (this.dic.is_solor) {
      H = tempo_1.Tempo.at(this.solar_hour_rule(), {
        write_at: utc,
        day: (0, tempo_1.envelope_of)(d),
      })
    ...
    // 年初来番号
    const w0 = tempo_1.Tempo.at(
      new tempo_1.FixedTempoRule(this.calc.msec.week, this.calc.zero.week),
      {
        write_at: u.last_at,
      },
    )
    ...
    const Y = (0, tempo_1.cyclic_label)(
      uEnvelope,
      u.now_idx + (yearEndsInNextYearsFirstWeek ? 1 : 0),
    )
    ...
    // 初期値定義と自己無矛盾になる。
    const a = (0, tempo_1.cyclic_label)(
      uEnvelope,
      (0, number_1.mod)(u.raw_now_idx - this.calc.zero.year60, this.dic.a.length),
    )
    const b = (0, tempo_1.cyclic_label)(
      uEnvelope,
      (0, number_1.mod)(u.raw_now_idx - this.calc.zero.year12, this.dic.b.length),
    )
    const c = (0, tempo_1.cyclic_label)(
      uEnvelope,
      (0, number_1.mod)(u.raw_now_idx - this.calc.zero.year10, this.dic.c.length),
    )
    const f = (0, tempo_1.cyclic_label)(
      uEnvelope,
      (0, number_1.mod)(u.raw_now_idx - this.calc.zero.year_s, this.dic.f.length),
    )
    // 月不断(四半期は月をまたぐため、区間は M(現在の月)のものをそのまま使う。
    // 四半期全体の境界ではない点に注意)
    const Q = (0, tempo_1.cyclic_label)(
      (0, tempo_1.envelope_of)(M),
      Math.floor((4 * M.now_idx) / this.dic.M.length),
    )
    // 日不断(固定 zero からの周期を length で割った余りをラベルにする)
    const A = tempo_1.Tempo.at(
      new tempo_1.CyclicDayTempoRule(this.calc.msec.day, this.calc.zero.day60, this.dic.A.length),
      {
        write_at: utc,
      },
    )
    const B = tempo_1.Tempo.at(
      new tempo_1.CyclicDayTempoRule(this.calc.msec.day, this.calc.zero.day12, this.dic.B.length),
      {
        write_at: utc,
      },
    )
    const C = tempo_1.Tempo.at(
      new tempo_1.CyclicDayTempoRule(this.calc.msec.day, this.calc.zero.day10, this.dic.C.length),
      {
        write_at: utc,
      },
    )
    const F = tempo_1.Tempo.at(
      new tempo_1.CyclicDayTempoRule(this.calc.msec.day, this.calc.zero.day_9, this.dic.F.length),
      {
        write_at: utc,
      },
    )
    ...
    if (this.is_table_leap) {
      // 旧暦では、週は月初にリセットする。
      E = tempo_1.Tempo.at(
        new tempo_1.CyclicDayTempoRule(this.calc.msec.day, this.calc.zero.week, this.dic.E.length),
        {
          write_at: utc,
        },
      )
      V = tempo_1.Tempo.at(
        new tempo_1.CyclicDayTempoRule(this.calc.msec.day, this.calc.zero.day28, this.dic.V.length),
        {
          write_at: utc,
        },
      )
      ...
      E = (0, tempo_1.cyclic_label)(
        dEnvelope,
        (0, number_1.mod)(M.now_idx + d.now_idx, this.dic.E.length),
      )
      V = (0, tempo_1.cyclic_label)(
        dEnvelope,
        (0, number_1.mod)(
