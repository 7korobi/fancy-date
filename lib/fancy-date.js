...
class FancyDate {
    ...
    span_between(from, to = Date.now(), { precise = false } = {}) {
        ...
        if (precise)
            return this.with_span_anchor(from, to, this.precise_span(from, to, precise));
        if (Math.abs(diff) < time_1.MINUTE) {
            return this.with_span_anchor(from, to, this.fixed_span(diff, 'second', time_1.SECOND, '秒'));
        }
        if (Math.abs(diff) < time_1.HOUR) {
            return this.with_span_anchor(from, to, this.fixed_span(diff, 'minute', time_1.MINUTE, '分'));
        }
        if (Math.abs(diff) < this.calc.msec.day) {
            return this.with_span_anchor(from, to, this.fixed_span(diff, 'hour', time_1.HOUR, '時間'));
        }
