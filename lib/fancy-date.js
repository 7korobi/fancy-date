...
exports.FancyDate = exports.prepareSpotModels = exports.prepareSpot = exports.transformOrbital = exports.TransformedOrbital = exports.MeanRotation = exports.MeanOrbital = exports.EarthSolarOrbital = exports.EarthMoonOrbital = void 0;
...
class FancyDate {
    ...
    span_obj(to, from, options = {}) {
        if (this.is_span_text(to, from))
            return this.relative_time_parse(to);
    ...
    relative_time_obj(text) {
        return this.relative_time_parse(text);
    }
    relative_time_add(utc, distance) {
        return this.add(utc, distance);
    }
    add_span(utc, span) {
        const anchor = span[relative_time_anchor];
    ...
    span_like_parts(span) {
        if ('string' === typeof span)
            return this.relative_time_parse(span).parts ?? [];
        if (Array.isArray(span))
            return span;
        return 'parts' in span ? (span.parts ?? [span]) : [span];
    }
    invert_span_like(span) {
        return this.span_like_parts(span).map(({ unit, value, label }) => ({ unit, value: 0 - value, label }));
    }
    parse_relative_time_part(text, sign) {
        let best;
        const accept = (unit, count, label) => {
            if (!label)
                return;
    ...
    relative_time_target_digits(utc, parts) {
        ...
        for (const { unit, value } of parts) {
            const token = relative_time_unit_token(unit);
    ...
    normalize_relative_time_target_digits(target) {
        ...
        if (relative_time_precision_rank('H') <= rank)
    ...
    relative_time_unit_msec(unit) {
        switch (unit) {
            ...
            case 'day':
                return time_1.DAY;
            case 'hour':
                return this.calc.msec.hour || time_1.HOUR;
            case 'minute':
                return this.calc.msec.minute || time_1.MINUTE;
            case 'second':
    ...
    find_time_by_relative_digits(target, utc) {
        if (target.changedRank < 0)
            return utc;
        const month = this.find_relative_time_month(target);
        const dayIndex = target.changedRank <= relative_time_precision_rank('M')
            ? Math.min(Math.max(target.d, 0), Math.max(0, Math.floor(month.size / time_1.DAY) - 1))
            : target.d;
        const day = this.to_tempos(month.last_at + dayIndex * time_1.DAY).d;
    ...
    find_relative_time_month(target) {
    ...
    compare_relative_time_digits(tempos, target) {
        const rows = [
    ...
    relative_time_interval_for_rank(tempos, rank) {
        if (rank <= relative_time_precision_rank('H'))
            return tempos.H;
        if (rank <= relative_time_precision_rank('m'))
            return tempos.m;
        if (rank <= relative_time_precision_rank('s'))
            return tempos.s;
        return tempos.S;
    }
    relative_time_source_since(target) {
        if (target.changedRank <= relative_time_precision_rank('H'))
            return target.sourceHourSince;
        if (target.changedRank <= relative_time_precision_rank('m'))
            return target.sourceMinuteSince;
        if (target.changedRank <= relative_time_precision_rank('s'))
            return target.sourceSecondSince;
        return 0;
    }
    clamp_relative_time_since(interval, since) {
    ...
    relative_time_distance(from, to = Date.now(), { precise = false } = {}) {
        const diff = to - from;
        if (!Number.isFinite(diff)) {
            return this.with_relative_time_anchor(from, to, { unit: 'year', value: NaN, label: '？？？' });
        }
        if (precise)
            return this.with_relative_time_anchor(from, to, this.relative_time_distance_precise(from, to, precise));
        if (Math.abs(diff) < time_1.MINUTE) {
            return this.with_relative_time_anchor(from, to, this.relative_time_distance_fixed(diff, 'second', time_1.SECOND, '秒'));
        }
        if (Math.abs(diff) < time_1.HOUR) {
            return this.with_relative_time_anchor(from, to, this.relative_time_distance_fixed(diff, 'minute', time_1.MINUTE, '分'));
        }
        if (Math.abs(diff) < this.calc.msec.day) {
            return this.with_relative_time_anchor(from, to, this.relative_time_distance_fixed(diff, 'hour', time_1.HOUR, '時間'));
        }
    ...
    with_relative_time_anchor(from, to, distance) {
    ...
    relative_time_distance_precise(from, to, precision) {
    ...
    relative_time_parts(from, to, precision) {
    ...
    relative_time_part_label(unit, count, fallbackUnit) {
    ...
    relative_time_distance_fixed(diff, unit, size, labelUnit) {
        const value = Math.trunc(Math.abs(diff) / size) * (diff < 0 ? -1 : 1);
        return { unit, value, label: relative_time_label(value, labelUnit) };
    }
    count_full_years(from, to, fromTempos, toTempos) {
        let count = this.count_tempo_boundaries('y', from, to);
        if (count && this.is_before_year_anniversary(fromTempos, toTempos))
            count--;
        return count;
    }
    count_full_months(from, to, fromTempos, toTempos) {
        let count = this.count_tempo_boundaries('M', from, to);
        if (count && toTempos.d.now_idx < fromTempos.d.now_idx)
            count--;
        return count;
    }
    is_before_year_anniversary(fromTempos, toTempos) {
    ...
    count_tempo_boundaries(unit, from, to) {
        const first = this.to_tempos(from)[unit];
        if (!first)
            return 0;
    ...
    span_from_options(from, options) {
        if (from == null)
    ...
    def_to_label() {
        ...
        function integer(idx) {
            return (_, val, size) => `${val.now_idx + idx}`.padStart(size, '0');
        }
        function at(cb) {
...
function relative_time_label(value, unit) {
    if (!value)
        return '今';
...
function relative_time_precision_rank(precision) {
    return 'yMdHmsS'.indexOf(precision);
}
function relative_time_unit_token(unit) {
    switch (unit) {
        case 'year':
            return 'y';
        case 'month':
