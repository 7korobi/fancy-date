...
class FancyDate {
    ...
    span_target(utc, parts) {
        ...
        for (const { unit, value } of parts) {
            const token = span_unit_token(unit);
    ...
    span_between(from, to = Date.now(), { precise = false } = {}) {
        ...
        if (Math.abs(diff) < time_1.MINUTE) {
            return this.with_span_anchor(from, to, this.fixed_span(diff, 'second', time_1.SECOND, '秒'), this.next_fixed_span_at(from, to, time_1.SECOND));
        }
        if (Math.abs(diff) < time_1.HOUR) {
            return this.with_span_anchor(from, to, this.fixed_span(diff, 'minute', time_1.MINUTE, '分'), this.next_fixed_span_at(from, to, time_1.MINUTE));
        }
        if (Math.abs(diff) < this.calc.msec.day) {
            return this.with_span_anchor(from, to, this.fixed_span(diff, 'hour', time_1.HOUR, '時間'), this.next_fixed_span_at(from, to, time_1.HOUR));
        }
        const parts = this.span_parts(from, to, 'd');
        for (const part of parts) {
            if (part.value) {
                return this.with_span_anchor(from, to, this.format_span([part], part.value < 0 ? '後' : '前'), this.to_tempos(to).d.next_at);
            }
        }
    ...
    next_fixed_span_at(target, at, size) {
        const diff = at - target;
        if (0 <= diff)
            return target + (Math.floor(diff / size) + 1) * size;
    ...
    fixed_span(diff, unit, size, labelUnit) {
...
function span_label(value, unit) {
    if (!value)
        return '今';
