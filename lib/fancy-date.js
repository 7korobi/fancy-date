...
exports.FancyDate = exports.rotationOf = exports.resolveSkyBody = exports.placeStar = exports.placeSatellite = exports.placePlanet = exports.orbitalOf = exports.isPlanetSkyBody = exports.hasSolarEvents = exports.hasLunarOrbitEvents = exports.hasLunarEvents = exports.centerOf = exports.bodyProfileOf = exports.prepareSpotModels = exports.prepareSpot = exports.transformOrbital = exports.TransformedOrbital = exports.MeanRotation = exports.MeanOrbital = exports.MarsSolarOrbital = exports.EarthSolarOrbital = exports.EarthMoonOrbital = void 0;
exports.tithi = tithi;
...
var orbital_model_2 = require("./orbital-model");
Object.defineProperty(exports, "bodyProfileOf", { enumerable: true, get: function () { return orbital_model_2.bodyProfileOf; } });
Object.defineProperty(exports, "centerOf", { enumerable: true, get: function () { return orbital_model_2.centerOf; } });
Object.defineProperty(exports, "hasLunarEvents", { enumerable: true, get: function () { return orbital_model_2.hasLunarEvents; } });
Object.defineProperty(exports, "hasLunarOrbitEvents", { enumerable: true, get: function () { return orbital_model_2.hasLunarOrbitEvents; } });
Object.defineProperty(exports, "hasSolarEvents", { enumerable: true, get: function () { return orbital_model_2.hasSolarEvents; } });
Object.defineProperty(exports, "isPlanetSkyBody", { enumerable: true, get: function () { return orbital_model_2.isPlanetSkyBody; } });
Object.defineProperty(exports, "orbitalOf", { enumerable: true, get: function () { return orbital_model_2.orbitalOf; } });
Object.defineProperty(exports, "placePlanet", { enumerable: true, get: function () { return orbital_model_2.placePlanet; } });
Object.defineProperty(exports, "placeSatellite", { enumerable: true, get: function () { return orbital_model_2.placeSatellite; } });
Object.defineProperty(exports, "placeStar", { enumerable: true, get: function () { return orbital_model_2.placeStar; } });
Object.defineProperty(exports, "resolveSkyBody", { enumerable: true, get: function () { return orbital_model_2.resolveSkyBody; } });
Object.defineProperty(exports, "rotationOf", { enumerable: true, get: function () { return orbital_model_2.rotationOf; } });
const year_cycle_tokens = ['yC60', 'yC12', 'yC10', 'yC9'];
const day_cycle_tokens = ['dC60', 'dC12', 'dC10', 'dC9', 'dC7', 'dC8', 'dC28'];
const calendar_note_tokens = ['R6', 'LM27'];
const cycle_tokens = [...year_cycle_tokens, ...day_cycle_tokens, ...calendar_note_tokens];
const year_cycle_zero_keys = {
    yC60: 'year60',
    yC12: 'year12',
    yC10: 'year10',
    yC9: 'year_s',
};
const day_cycle_zero_keys = {
    dC60: 'day60',
    dC12: 'day12',
    dC10: 'day10',
    dC9: 'day_9',
...
// (確認済み)ため、この拡張は後方互換。
const reg_token = new RegExp(`(${token_word_formats}|[EHMNQZdmsy][or]|([${regexp_token_chars}])\\2*)|''|'(''|[^'])+('|$)|.`, 'g');
function is_legacy_token_alias(token) {
    return token in legacy_token_aliases;
}
function is_cycle_token(token) {
    return cycle_tokens.includes(token);
}
function is_year_cycle_token(token) {
    return year_cycle_tokens.includes(token);
}
function is_day_cycle_token(token) {
    return day_cycle_tokens.includes(token);
}
function is_calendar_note_token(token) {
    return calendar_note_tokens.includes(token);
}
function token_base(token) {
    return is_legacy_token_alias(token) ? legacy_token_aliases[token] : token;
}
function define_token_alias(target, alias, source) {
...
function sync_legacy_token_aliases(target) {
    for (const legacy of Object.keys(legacy_token_aliases)) {
        define_token_alias(target, legacy, legacy_token_aliases[legacy]);
    }
    return target;
}
function format_token_parts(token) {
    if (token === 'Zz')
        return { top: 'Zz', mode: '', size: 1 };
    ...
    if (top && all_tokens.includes(top)) {
        return { top: top, mode, size: token.length };
    }
    return null;
}
const seasonal_note_label_map = Symbol('seasonal_note_label_map');
const default_zassetsu_note_labels = {
    春彼岸: '彼岸',
    秋彼岸: '彼岸',
    春社日: '社日',
    秋社日: '社日',
...
function with_seasonal_note_labels(notes, labels) {
...
function tithi() {
    return (_dayStart, { calendar, at, previousAt, nextAt }) => {
        ...
        if (!moony)
            throw new Error('tithi() assignment requires a satellite orbital model');
        const rawAt = (targetAt) => {
            const now_idx = Math.min(29, Math.floor((0, number_1.mod)(moony.phaseAt(targetAt), 1) * 30));
        ...
        const now_idx = (0, number_1.mod)(assignment_raw_now_idx, 30);
...
function to_indexs(zero) {
    ...
    return sync_legacy_token_aliases(data);
}
class AssignedTempoRule {
    constructor(innerRule, token, calendar, assignment, currentDayStart) {
    ...
    assign(raw, base) {
        ...
        if (!Number.isFinite(now_idx))
            throw new Error(`invalid assignment index ${now_idx}`);
        return {
            ...rawEnvelope,
            now_idx,
...
class FancyDate {
    constructor(baseOrBuild, build) {
        ...
        if (create)
            return FancyDate.lazy(() => create(new FancyDate(base)).init());
    ...
    static lazy(create) {
        ...
        const target = Object.create(FancyDate.prototype);
    ...
    calendar(start = ['1970-1-1 0:0:0', 'y-M-d H:m:s', 0], leaps = null, month_divs = null, leap_shift = 0) {
    ...
    init() {
        ...
        // 冬至付近で退化するケースは残る)。
        if ((this.dic.is_solor || this.day_start_event()) &&
            this.dic.geo &&
            66.5 <= Math.abs(this.dic.geo[0])) {
            throw new Error(`不定時法(division({ H: 'solar' }))・太陽イベント起点の暦日(dayStart())は極域(緯度${this.dic.geo[0]}度)では成立しません。極圏(66.5度)以遠では日の出・日の入りが存在しない期間が生じ、これらの前提が崩れます。`);
        }
    ...
    lunisolar(utc) {
        ...
        for (let i = 0; i < cache.length; i++) {
            ...
            if (cached.last_at <= utc && utc < cached.next_at) {
                ...
                const day = (0, time_1.to_tempo_bare)(this.calc.msec.day, this.calc.zero.day, utc);
                return {
                    ...cached,
        ...
        const resolved = (0, lunisolar_1.lunisolar)({
    ...
    parse_span(text, options = {}) {
        const { parts, direction } = this.parse_span_parts(text);
        const span = this.format_span_parts(parts, direction);
        if (options.at != null)
            this.with_span_anchor_at(this.to_utc(options.at), span);
        return span;
    }
    format_span(span, direction) {
    ...
    span_msec(span, options = {}) {
        const anchor = 'string' === typeof span ? undefined : span[span_anchor];
        if (anchor?.calendar === this && anchor.msec != null)
            return anchor.msec;
        const at = options.at != null
            ...
            : anchor?.calendar === this
                ...
                : undefined;
        if (at == null)
            throw new Error('span_msec() requires an anchor time');
    ...
    parse_span_parts(text) {
        const source = text.trim();
        if (source === '今')
            return { parts: [], direction: '前' };
        ...
        const direction = (match ? match[2] : '後');
        if (!body)
    ...
    disambiguate_span_parts(parts) {
        if (this.span_part_fallback_unit('d') !== this.span_part_fallback_unit('D'))
            return parts;
        const hasYear = parts.some(({ token }) => token === 'y' || token === 'Y');
        const hasMonth = parts.some(({ token }) => token === 'M');
        if (!hasYear || hasMonth)
            return parts;
        return parts.map((part) => (part.token === 'd' ? { ...part, token: 'D' } : part));
    }
    format_span_parts(parts, direction) {
        ...
        const label = direction || signs.size === 1
    ...
    merge_span_parts(left, right) {
        const merged = new Map();
        const seen = new Map();
        const addPart = (part, index) => {
        ...
        let index = 0;
        for (const part of this.span_parts_of(left))
            addPart(part, index++);
        for (const part of this.span_parts_of(right))
            addPart(part, index++);
    ...
    span_anchor_at(span) {
        if ('string' === typeof span || Array.isArray(span))
            return undefined;
        const anchor = span[span_anchor];
        return anchor?.calendar === this ? anchor.at : undefined;
    }
    merge_span_anchor_at(left, right) {
    ...
    span_target(utc, parts) {
        ...
        for (const { token: spanToken, unit, value } of parts) {
            let token = (spanToken ?? span_unit_token(unit));
    ...
    resolve_span_week_target(target) {
        if (target.week == null)
            return;
    ...
    find_span_month(target) {
        ...
        if (near.u.raw_now_idx === target.u &&
    ...
    find(between, conditions, options = {}) {
    ...
    infer_find_step_from_format(format) {
        let step = 'y';
        const tokens = format.match(reg_token) ?? [];
        for (const token of tokens) {
            const candidate = this.find_step_for_token((format_token_parts(token)?.top ?? token[0]));
    ...
    find_step_for_token(token) {
        const base = token_base(token);
        if (is_day_cycle_token(base) || is_calendar_note_token(base))
            return 'd';
        if (is_year_cycle_token(base))
            return 'y';
        switch (base) {
            case 'S':
                return 'S';
    ...
    span_between(from, to = Date.now(), { precise = false } = {}) {
        ...
        for (const part of parts) {
            if (part.value) {
                return this.with_span_anchor(from, to, this.format_span_parts([part], part.value < 0 ? '後' : '前'), toTempos.d.next_at);
            }
        }
        return this.with_span_anchor(from, to, { unit: 'day', value: 0, label: '今', parts: [] }, toTempos.s.next_at);
    }
    with_span_anchor(target, at, span, next_at) {
        if (Number.isFinite(next_at) && at < next_at) {
            span.next_at = next_at;
            span.timeout = next_at - at;
        }
        Object.defineProperty(span, span_anchor, {
            value: { calendar: this, at, msec: target - at },
            enumerable: false,
        });
        return span;
    }
    with_span_anchor_at(at, span) {
        Object.defineProperty(span, span_anchor, {
            value: { calendar: this, at },
            enumerable: false,
        });
        return span;
    }
    ...
    hierarchical_span_rows(precision, earlierTempos, laterTempos) {
        const coreRows = [
    ...
    span_part_fallback_unit(token) {
        return this.dic.labels[token] ?? String(token);
    }
    span_part_label(token, count, fallbackUnit) {
    ...
    is_tempos(utc) {
        return (!!utc &&
            'object' === typeof utc &&
            typeof utc.d?.last_at === 'number' &&
            typeof utc.M?.last_at === 'number');
    }
    is_date_range(value) {
    ...
    week_cycle_token() {
        ...
        const choices = ['dC8', 'dC10', 'dC7'];
        for (const choice of choices) {
            if (tokens.some((token) => format_token_parts(token)?.top === choice))
                return choice;
        }
        for (const choice of choices) {
            if (this.dic.E === this.dic[choice])
                return choice;
        }
        return 'dC7';
    }
    ...
    def_year_table() {
        ...
        if (period) {
            ...
            for (let idx = 0; idx < period; idx++) {
                const rel_idx = (0, number_1.mod)(idx - leap_shift, period);
                let is_leap = 0;
            ...
            range.year[(0, number_1.mod)(leap_shift, period)] = this.calc.range.year[1];
        }
        else {
    ...
    def_zero() {
        ...
        const rawX = (0, time_1.to_tempo_bare)(this.calc.msec.hour, -0.5 * this.calc.msec.hour, timezone);
        this.dic.x.tempo = (0, tempo_1.cyclic_label)((0, tempo_1.envelope_of)(rawX), timezone);
    ...
    precision() {
        ...
        return {
            ...
            is_legal_eto: is_just(this.dic.yCS.length, this.dic.yC.length) &&
                is_just(this.dic.yCB.length, this.dic.yC.length),
            is_legal_ETO: is_just(this.dic.dCS.length, this.dic.dC.length) &&
  ...
  */
    ...
    noon(utc, day) {
        ...
        day ??= (0, time_1.to_tempo_bare)(this.calc.msec.day, this.calc.zero.day, utc);
    ...
    雑節(utc, { Zz, d } = this.to_tempos(utc)) {
        if ((0, orbital_model_1.hasSolarEvents)(this.dic.sunny))
            return this.雑節_by_phase(utc);
        return with_seasonal_note_labels((0, solar_1.雑節_by_mean)(Zz, d, this.calc.msec.day, this.calc.zero.day10, this.dic.dCS.length), default_zassetsu_note_labels);
    }
    雑節_by_phase(utc) {
        return with_seasonal_note_labels((0, solar_1.雑節_by_phase)(this.dic.sunny, this.calc.msec.day, this.calc.zero.day, this.calc.zero.day10, this.dic.dCS.length, utc), default_zassetsu_note_labels);
    }
     ...
     */
    resolve_orbital_season(utc) {
        return tempo_1.Tempo.at(this.orbital_season_rule(), { write_at: utc });
    }
    /**
     ...
     */
    orbital_season_rule() {
        return (this._orbital_season_rule ??= new tempo_1.CachedTempoRule(new tempo_1.OrbitalPhaseTempoRule(this.dic.sunny, this.dic.Z.length, 1 / 8)));
    }
    /**
     ...
     */
    solar_hour_rule() {
        return (this._solar_hour_rule ??= new tempo_1.CachedTempoRule(new tempo_1.SolarDayHourTempoRule(this.dic.sunny, this.dic.earthy, this.dic.geo, this.calc.msec.day, this.calc.zero.day, this.calc.msec.year, this.calc.zero.season, this.dic.H.length)));
    }
    day_start_event() {
        const dayStart = this.dic.day_start;
        if (dayStart === 'sunrise' || dayStart === 'sunset')
            return dayStart;
        if (this.dic.is_dusk)
            return 'sunset';
        return undefined;
     ...
     */
    solar_event_day_core_rule() {
        const event = this.day_start_event();
        if (!event)
            throw new Error('dayStart() is not configured for a solar event');
        return (this._solar_event_day_core_rule ??= new tempo_1.SolarEventDayTempoRule(this.dic.sunny, this.dic.earthy, this.dic.geo, this.calc.msec.day, this.calc.zero.day, this.calc.msec.year, this.calc.zero.season, event));
    }
    solar_event_day_rule() {
        return (this._solar_event_day_rule ??= new tempo_1.CachedTempoRule(this.solar_event_day_core_rule(), (base) => base.parent.last_at));
    }
    align_day_start_month_start(rawStart) {
        return this.solar_event_day_core_rule().boundary_at_or_after(rawStart);
    }
    month_rule(rule) {
        return this.day_start_event()
            ? new tempo_1.StartAlignedTempoRule(rule, (rawStart) => this.align_day_start_month_start(rawStart))
            : rule;
    }
    year_rule(rule) {
        return this.day_start_event()
    ...
    day_rule() {
        if (this.day_start_event())
            return this.solar_event_day_rule();
        const offset = this.dic.day_offset_hours
            ? (this.dic.day_offset_hours / 24) * this.calc.msec.day
            : 0;
        return new tempo_1.SubdivideTempoRule(this.calc.msec.day, offset);
    }
    assign_day_tempo(day) {
        const assignment = this.dic.assignments.d;
        if (!assignment)
            return day;
        const rule = new AssignedTempoRule(day.rule, 'd', this, assignment, () => this.current_day_start());
        return new tempo_1.Tempo(rule.assign((0, tempo_1.envelope_of)(day), day.base), day.base, rule);
    }
    ...
    date_note_labels(groups, tempos) {
        const list = [];
    ...
    to_tempos(utc) {
        ...
        if (!Number.isFinite(utc))
            throw new Error(`invalid timestamp ${utc}`);
        const J = tempo_1.Tempo.at(new tempo_1.FixedTempoRule(this.calc.msec.day, this.calc.zero.jd), {
            write_at: utc,
        }); // ユリウス日
        // season in year_of_planet
        const Zz = tempo_1.Tempo.at(new tempo_1.FixedTempoRule(this.calc.msec.year, this.calc.zero.season), {
            write_at: utc,
        }); // 太陽年
        // 実軌道(sunny.timeOfPhase)を持つ場合は定気法(実際の黄経)で二十四節気を解決する。
        // そうでなければ従来通り平気法(等角分割、SubdivideTempoRule)のまま。
        const usesOrbitalSeasons = (0, orbital_model_1.hasSolarEvents)(this.dic.sunny);
        const seasonRule = new tempo_1.SubdivideTempoRule(this.calc.msec.season);
        const ZzEnvelope = (0, tempo_1.envelope_of)(Zz);
        const resolve_season = (at) => usesOrbitalSeasons
            ? this.resolve_orbital_season(at)
            : tempo_1.Tempo.at(seasonRule, { write_at: at, parent: ZzEnvelope });
        ...
        if (this.dic.moony && moon_msec != null && !usesObservedLunisolar) {
            ...
            // される不具合があった)。
            Nn = tempo_1.Tempo.at(new tempo_1.MeanLunisolarMonthRule(moon_msec, this.calc.zero.moon, this.calc.msec.day, this.calc.zero.day, this.dic.Z.length, resolve_season), { write_at: utc });
            ...
            // 表すため dusk() を適用する。
            N = tempo_1.Tempo.at(this.is_table_month ? new tempo_1.SubdivideTempoRule(this.calc.msec.day) : this.day_rule(), { write_at: utc, parent: (0, tempo_1.envelope_of)(Nn) });
        }
        if (this.is_table_leap) {
            p = tempo_1.Tempo.at(new tempo_1.FixedTempoRule(this.calc.msec.period, this.calc.zero.period), {
                write_at: utc,
            });
            ...
            const yearRule = new tempo_1.TableTempoRule(this.table.msec.year, this.calc.zero.period);
            ...
            u = tempo_1.Tempo.at(this.year_rule(yearRule), {
                write_at: utc,
            });
            M = tempo_1.Tempo.at(this.month_rule(new tempo_1.TableTempoRule(this.table.msec.month[yearSize], rawYear.last_at)), {
                write_at: utc,
            });
            d = tempo_1.Tempo.at(this.day_rule(), {
                write_at: utc,
                parent: (0, tempo_1.envelope_of)(M),
            });
        }
        else {
            if (this.is_table_month) {
                const yearRule = new tempo_1.FloorTempoRule(this.calc.msec.year, this.calc.zero.spring, [
                ...
                const yearSize = rawYear.next_at - rawYear.last_at;
                u = tempo_1.Tempo.at(this.year_rule(yearRule), { write_at: utc });
                M = tempo_1.Tempo.at(this.month_rule(new tempo_1.TableTempoRule(this.table.msec.month[yearSize], rawYear.last_at)), {
                    write_at: utc,
                });
                d = tempo_1.Tempo.at(this.day_rule(), {
                ...
                if (usesObservedLunisolar) {
                    ...
                    // 年境界でリセットされる。
                    u = tempo_1.Tempo.at(this.year_rule(new tempo_1.EraAdjustedTempoRule(new tempo_1.ObservedLunisolarYearRule((at) => this.lunisolar(at)), this.calc.msec.year, this.table.msec.era, this.calc.zero.era, this.calc.eras)), { write_at: utc });
                    M = tempo_1.Tempo.at(this.month_rule(new tempo_1.ObservedLunisolarMonthRule((at) => this.lunisolar(at), moon_msec)), {
                        write_at: utc,
                    });
                    ...
                    u = tempo_1.Tempo.at(this.year_rule(new tempo_1.EraAdjustedTempoRule(new tempo_1.FloorTempoRule(this.calc.msec.year, this.calc.zero.season + this.calc.msec.season, [
                        { size: moon_msec, zero: this.calc.zero.moon },
                        { size: this.calc.msec.day, zero: this.calc.zero.day },
                    ]), this.calc.msec.year, this.table.msec.era, this.calc.zero.era, this.calc.eras)), { write_at: utc });
                    if (this.dic.is_dusk) {
                        M = tempo_1.Tempo.at(this.month_rule(new tempo_1.MeanLunisolarMonthRule(moon_msec, this.calc.zero.moon, this.calc.msec.day, this.calc.zero.day, this.dic.Z.length, resolve_season)), { write_at: utc });
                        d = tempo_1.Tempo.at(this.day_rule(), {
                            write_at: utc,
                            parent: (0, tempo_1.envelope_of)(M),
                        });
        ...
        if (this.dic.is_solor) {
            H = tempo_1.Tempo.at(this.solar_hour_rule(), { write_at: utc, day: (0, tempo_1.envelope_of)(d) });
            m = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(H.size / this.dic.m.length), {
                write_at: utc,
                parent: (0, tempo_1.envelope_of)(H),
            });
        }
        else {
            H = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.hour), {
                write_at: utc,
                parent: (0, tempo_1.envelope_of)(d),
            });
            m = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.minute), {
                write_at: utc,
                parent: (0, tempo_1.envelope_of)(H),
            });
        }
        const s = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.second), {
            write_at: utc,
            parent: (0, tempo_1.envelope_of)(m),
        });
        const S = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.msec), {
            write_at: utc,
            parent: (0, tempo_1.envelope_of)(s),
        });
        ...
        // `{}` キャストで型を偽ることは避ける)。
        let G = (0, tempo_1.cyclic_label)((0, tempo_1.envelope_of)(u), 0);
        if (this.table.msec.era != null) {
            G = tempo_1.Tempo.at(new tempo_1.TableTempoRule(this.table.msec.era, this.calc.zero.era), {
                write_at: utc,
            });
        ...
        const uEnvelope = (0, tempo_1.envelope_of)(u);
        // 年初来番号
        const w0 = tempo_1.Tempo.at(new tempo_1.FixedTempoRule(this.calc.msec.week, this.calc.zero.week), {
            write_at: u.last_at,
        });
        const w = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.week), {
            write_at: utc,
            parent: (0, tempo_1.envelope_of)(w0),
        });
        // D(年初来日数)は d(月内日)とは別の関心事(年内の通し日数)なので、
        // dusk()/dayBoundary() の対象にしない(常に実時計 0 時起点のまま)。
        const D = tempo_1.Tempo.at(new tempo_1.SubdivideTempoRule(this.calc.msec.day), {
            write_at: utc,
            parent: uEnvelope,
        });
        // 年末最終週は、翌年初週の扱いにする。Y はその調整を反映した年ラベル。
        const yearEndsInNextYearsFirstWeek = u.next_at < w.next_at;
        const Y = (0, tempo_1.cyclic_label)(uEnvelope, u.now_idx + (yearEndsInNextYearsFirstWeek ? 1 : 0));
        ...
        for (const token of year_cycle_tokens) {
            ...
            yearCycles[token] = (0, tempo_1.cyclic_label)(uEnvelope, (0, number_1.mod)(u.raw_now_idx - zero, this.dic[token].length));
        }
        const { yC60, yC12, yC10, yC9 } = yearCycles;
        // 月不断(四半期は月をまたぐため、区間は M(現在の月)のものをそのまま使う。
        // 四半期全体の境界ではない点に注意)
        const Q = (0, tempo_1.cyclic_label)((0, tempo_1.envelope_of)(M), Math.floor((4 * M.now_idx) / this.dic.M.length));
        // 日不断(固定 zero からの周期を length で割った余りをラベルにする)
        const dayCycles = {};
        for (const token of day_cycle_tokens) {
            const zero = this.calc.zero[day_cycle_zero_keys[token]];
            dayCycles[token] = tempo_1.Tempo.at(new tempo_1.CyclicDayTempoRule(this.calc.msec.day, zero, this.dic[token].length), {
                write_at: utc,
            });
        }
        ...
        const dEnvelope = (0, tempo_1.envelope_of)(d);
        const R6 = (0, tempo_1.cyclic_label)(dEnvelope, this.dic.R6.length ? (0, number_1.mod)(M.now_idx + d.now_idx, this.dic.R6.length) : 0);
        const LM27 = (0, tempo_1.cyclic_label)(dEnvelope, this.dic.LM27.length
            ? (0, number_1.mod)([11, 13, 15, 17, 19, 21, 24, 0, 2, 4, 7, 9][M.now_idx] + d.now_idx, this.dic.LM27.length)
            : 0);
    ...
    get_dic(tgt, tokens, reg) {
        ...
        if (dayPeriod != null) {
            ...
            data.H = (0, number_1.mod)(data.H, half) + dayPeriod * half;
        }
        return data;
    }
    index(src, str = this.dic.parse, _disuse = 0) {
        ...
        if (parsedTokens.has('yC60')) {
            data.yC10 = (0, number_1.mod)(data.yC60, this.dic.yC10.length);
            data.yC12 = (0, number_1.mod)(data.yC60, this.dic.yC12.length);
        }
        if (parsedTokens.has('dC60')) {
            data.dC10 = (0, number_1.mod)(data.dC60, this.dic.dC10.length);
            data.dC12 = (0, number_1.mod)(data.dC60, this.dic.dC12.length);
        }
    ...
    parse_by(data, diff = {}) {
        ...
        if (this.is_table_leap) {
        ...
        else if (!this.is_table_month &&
            (0, orbital_model_1.hasSolarEvents)(this.dic.sunny) &&
            (0, orbital_model_1.hasLunarEvents)(this.dic.moony)) {
            ...
            const yearEnvelope = new tempo_1.FloorTempoRule(this.calc.msec.year, zero, [
                { size: this.calc.msec.day, zero: this.calc.zero.day },
        ...
        if (this.is_table_month) {
            ...
            last_at = new tempo_1.FloorTempoRule(moon_msec, this.calc.zero.moon, [
                { size: this.calc.msec.day, zero: this.calc.zero.day },
            ]).at(M_utc).last_at;
            utc += last_at - base;
        }
     ...
     */
    resolve_day_start(month_start, d) {
        const event = this.day_start_event();
        if (!event) {
            const offset = this.dic.day_offset_hours
                ? (this.dic.day_offset_hours / 24) * this.calc.msec.day
        ...
        const rule = new tempo_1.SolarEventDayTempoRule(this.dic.sunny, this.dic.earthy, this.dic.geo, this.calc.msec.day, this.calc.zero.day, this.calc.msec.year, this.calc.zero.season, event);
        const parent = {
            zero: month_start,
            now_idx: 0,
            last_at: month_start,
    ...
    half_label(now_idx, length, labels) {
        const side = (0, number_1.mod)(Math.floor((now_idx * 2) / length), 2);
        return labels[side];
    }
    paksha_label(tempos) {
        if (!tempos.N)
    ...
    format_parts_by(utc, str = this.dic.format) {
        ...
        return tokens.map((token) => {
            ...
            const withRuby = (text) => ruby ? { token, text, ruby } : { token, text };
            switch (mode) {
                case 'r':
