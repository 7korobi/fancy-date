...
class FancyDate {
    ...
    init() {
        ...
        // 冬至付近で退化するケースは残る)。
        if ((this.dic.is_solor || this.dic.is_dusk) &&
            this.dic.geo &&
            66.5 <= Math.abs(this.dic.geo[0])) {
            throw new Error(`不定時法(division({ H: 'solar' }))・日没起点の暦日(dusk())は極域(緯度${this.dic.geo[0]}度)では成立しません。極圏(66.5度)以遠では日の出・日の入りが存在しない期間が生じ、これらの前提が崩れます。`);
        }
  ...
  */
    ...
    * d/N(dusk() による日没起点の暦日)で使う SolarEventDayTempoRule('sunset') を
     ...
     */
    real_sunset_day_core_rule() {
        return (this._real_sunset_day_core_rule ??= new tempo_1.SolarEventDayTempoRule(this.dic.sunny, this.dic.earthy, this.dic.geo, this.calc.msec.day, this.calc.zero.day, this.calc.msec.year, this.calc.zero.season, 'sunset'));
    }
    real_sunset_day_rule() {
        return (this._real_sunset_day_rule ??= new tempo_1.CachedTempoRule(this.real_sunset_day_core_rule(), (base) => base.parent.last_at));
    }
    align_dusk_month_start(rawStart) {
        return this.real_sunset_day_core_rule().boundary_at_or_after(rawStart);
    }
    month_rule(rule) {
        return this.dic.is_dusk
            ? new tempo_1.StartAlignedTempoRule(rule, (rawStart) => this.align_dusk_month_start(rawStart))
            : rule;
    }
    year_rule(rule) {
        return this.dic.is_dusk
    ...
    day_rule() {
        if (this.dic.is_dusk)
            return this.real_sunset_day_rule();
    ...
    * parent を誤って使い回す実バグが
    ...
    resolve_day_start(month_start, d) {
        ...
        const rule = new tempo_1.SolarEventDayTempoRule(this.dic.sunny, this.dic.earthy, this.dic.geo, this.calc.msec.day, this.calc.zero.day, this.calc.msec.year, this.calc.zero.season, 'sunset');
        const parent = {
            zero: month_start,
            now_idx: 0,
            last_at: month_start,
