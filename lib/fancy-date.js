...
class FancyDate {
    ...
    lunisolar(utc) {
        ...
        return (this._lunisolar_cache = (0, lunisolar_1.lunisolar)({
  ...
  */
    ...
    resolve_orbital_season(utc) {
        ...
        const now_idx = (0, number_1.mod)(Math.round((0, number_1.mod)(this.dic.sunny.phaseAt(env.last_at) + referencePhaseOffset, 1) * divisions), divisions);
        const zero = env.last_at - now_idx * avgSize;
        const tempo = new time_1.Tempo(zero, now_idx, utc, env.last_at, env.next_at);
        tempo.path = 'season';
        return tempo;
    }
    /**
    ...
    to_tempos(utc) {
        ...
        if (this.dic.moony && moon_msec != null && !usesObservedLunisolar) {
            ...
            const NnEnvelope = new tempo_model_1.MeanLunarPhaseTempoRule(moon_msec, this.calc.zero.moon, this.calc.msec.day, this.calc.zero.day).at(utc);
            Nn = new time_1.Tempo(NnEnvelope.zero, NnEnvelope.now_idx, utc, NnEnvelope.last_at, NnEnvelope.next_at);
        ...
        if (this.is_table_leap) {
            p = to_tempo('period');
            u = tempo_model_1.TempoView.at(new tempo_model_1.TableTempoRule(this.table.msec.year, p.last_at), { write_at: utc });
        ...
        const D = tempo_model_1.TempoView.at(new tempo_model_1.SubdivideTempoRule(this.calc.msec.day), {
            write_at: utc,
            parent: (0, tempo_model_1.envelope_of)(u),
        });
