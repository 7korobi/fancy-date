...
class SolarDayHourTempoRule {
  ...
  hour_table(write_at, dayEnvelope) {
    ...
    const solarNoon = (0, solar_1.noon)(
      this.sunny,
      this.dayMsec,
      this.dayZero,
      this.yearMsec,
...
class OrbitalPhaseTempoRule {
  ...
  at(write_at) {
    ...
    const boundaryAt = (idx) =>
      (0, solar_1.solar_phase)(
        this.sunny,
        idx / this.divisions - this.referencePhaseOffset,
        write_at,
      )
    let idx = Math.floor(
      (0, number_1.mod)(this.sunny.phaseAt(write_at) + this.referencePhaseOffset, 1) *
        this.divisions,
    )
