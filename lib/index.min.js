...
({
  ...
  to_tempo_by,
  to_tempo_bare
} = __webpack_require__(/*! ./time */ "./src/time.ts"));
...
var FancyDate = class FancyDate {
  ...
  era(era, eras = []) {
    new Indexer(this.dic, 'G', '', 0, ["紀元前", ...eras.map(([s]) => {
      return s;
    })]);
    Object.assign(this.dic, {
      era,
  ...
  calendar(start, start_at, leaps = null, month_divs = null) {
    ...
    this.is_table_month = month_divs != null;
    return this;
  }

  rolls(weeks, etos) {
    new Indexer(this.dic, 'E', ...weeks);
    new Indexer(this.dic, 'T', ...etos);
    return this;
  }

  yeary(months, days) {
    new Indexer(this.dic, 'M', ...months);
    new Indexer(this.dic, 'd', ...days);
    return this;
  }

  moony(moons) {
    new Indexer(this.dic, 'N', ...moons);
    return this;
  }

  seasonly(seasons) {
    new Indexer(this.dic, 'Z', ...seasons);
    return this;
  }

  daily(hours, minutes, seconds, is_solor = false) {
    new Indexer(this.dic, 'H', ...hours);
    new Indexer(this.dic, 'm', ...minutes);
    new Indexer(this.dic, 's', ...seconds);
  ...
  init() {
    ...
    season = sub_define(this.calc.msec.year, this.dic.Z.length);
    month = daily_measure(this.calc.msec.year / this.dic.M.length, this.calc.msec.day);
    week = daily_define(this.dic.E.length * this.calc.msec.day, this.calc.msec.day);
    hour = sub_define(this.calc.msec.day, this.dic.H.length);

    if (this.dic.is_solor) {
      minute = sub_define(hour.msec, this.dic.m.length);
  ...
  def_table_by_leap_day() {
    var a, day, div, i, idx, is_leap, j, k, l, leaps, len, len1, len2, mode, month, month_divs, month_sum, period, range, ref, size, str, upto, year, years;
  ...
  def_table() {
    return this.get_table = this.is_table_leap ? this.def_table_by_leap_day() : this.is_table_month ? this.def_table_by_leap_month() : this.def_table_by_season();
  }

  def_idx() {
    var day, eto, full_period, hour, minute, month, moon, period, ref, ref1, ref2, ref3, ref4, ref5, season, second, week, year;
    week = (ref = this.dic.E) != null ? ref.length : void 0;
    eto = (ref1 = this.dic.T) != null ? ref1.length : void 0;
    ...
    minute = minute - 0;
    second = second - 0;
    eto = (ref2 = this.dic.T) != null ? ref2.idx : void 0;
    week = (ref3 = this.dic.E) != null ? ref3.idx : void 0;
    season = (ref4 = this.dic.Z) != null ? ref4.idx : void 0;
    moon = 0;

    if (this.is_table_leap) {
      ref5 = this.dic.leaps, [full_period] = slice.call(ref5, -1);
      period = full_period;
      Object.assign(this.calc.divs, {
        period
      });
  ...
  def_zero() {
    var day, era, hour, jd, ld, minute, mjd, month, moon, period, ref, season, second, since, tz_offset, week, year, year_size, zero, zero_size;

    zero_size = (path, idx = 0) => {
      return 0 - (this.calc.idx[path] - idx) * this.calc.msec[path];
    };

    tz_offset = this.dic.earthy[0] / 360 * this.dic.geo[1];
    zero = this.dic.start_at - tz_offset;
    ...
    season = this.dic.sunny[1] + zero_size("season"); // 立春点
  ...
  */
  ...
  solor(utc, idx = 2, {
    ...
    K = this.dic.earthy[2] * deg_to_rad;
    lat = this.dic.geo[0] * deg_to_rad;
    ...
    T1 = to_tempo_bare(this.calc.msec.year, this.dic.sunny[1], 南中時刻);
    spring = T1.last_at;
  ...
  to_tempos(utc) {
    var D, E, G, H, J, M, N, N0, N0_p, Nn, S, T, Y, Z, Zs, Zz, center_at, d, drill_down, e, era, era_base, m, now_idx, p, s, size, to_tempo, to_tempo_mod, to_tempo_mod2, u, w, w0, y;
    ...
    switch (Zs.now_idx >> 1) {
      ...
      case this.dic.Z.length >> 1:
        // 太陽年末に13月が出てしまう。年初にする。
        Zz = to_tempo_bare(Zz.size, Zz.zero, Zs.next_at);
    }

    Nn.now_idx = modulo(Zs.now_idx, this.dic.Z.length) >> 1;
    N = drill_down(Nn, 'day');
    ...
    if (!this.is_table_leap) {
      // 旧暦では、週は月初にリセットする。
      e.now_idx = (M.now_idx + d.now_idx) % this.dic.E.length;
    ...
    T = {
      now_idx: (u.now_idx + this.calc.idx.eto) % this.dic.T.length
    };
    G = {};
  ...
  parse(tgt, str = default_parse_format) {
    ...
    return d * this.calc.msec.day + H * this.calc.msec.hour + m * this.calc.msec.minute + s * this.calc.msec.second + S + (this.is_table_leap ? (size = this.table.range.year[y] * this.calc.msec.day, this.calc.zero.period + p * this.calc.msec.period + (this.table.msec.year[y - 1] || 0) + (this.table.msec.month[size][M - 1] || 0)) : this.calc.zero.season + y * this.calc.msec.year + M * this.calc.msec.month);
  }
...
日本 = [35, 135];
地球 = [[31556925147.0, new Date("2019/03/21 06:58").getTime()], [2551442889.6, new Date("2019/01/06 10:28").getTime()], [86400000, 0, 23.4397 // LOD ではなく、暦上の1日。Unix epoch では閏秒を消し去るため。
], 日本];
火星 = [[59354347573.5373, new Date("2018/10/28 09:00").getTime()], null, [88775000, 0, 25.19 // 24時間39分35秒。
], [35, 0]];
GREGORIO = {
...
[315569251.470, new Date("2019/03/21 06:58").getTime() / 100], [25514428.896, new Date("2019/01/06 10:28").getTime() / 100], [864000, 0, 23.4397 // LOD ではなく、暦上の1日。Unix epoch では閏秒を消し去るため。
], 日本];
