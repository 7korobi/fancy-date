...
exports.noon = noon;
exports.solor = solor;
exports.solar_terms_mean = solar_terms_mean;
exports.雑節_from_terms = 雑節_from_terms;
exports.雑節_by_mean = 雑節_by_mean;
...
function solar_term(sunny, dayMsec, dayZero, utc, phase) {
    ...
    return tempo_1.Tempo.at(new tempo_1.FixedTempoRule(dayMsec, dayZero), { write_at: at });
}
 ...
 */
function solar_terms_mean(Zz, d) {
    ...
    const term = (phase) => {
        ...
        return tempo_1.Tempo.at(new tempo_1.FixedTempoRule(d.size, d0.last_at), { write_at: now });
    };
    return {
        立春: term(phases.立春),
        入梅: term(phases.入梅),
 ...
 */
function 雑節_from_terms(dayMsec, day10Zero, stemLength, terms) {
    let { 立春, 入梅, 春分, 半夏生, 夏土用, 立夏, 夏至, 秋土用, 立秋, 秋分, 冬土用, 立冬, 冬至, 春土用, 次立春: 立春2, } = terms;
    const [八十八夜, 二百十日, 二百二十日] = [88, 210, 220].map((n) => 立春.succ(n - 1));
    const [春彼岸, 秋彼岸] = [春分, 秋分].map((dd) => {
        return (0, tempo_1.join)(dd.back(3), dd.succ(3));
    });
    const [春社日, 秋社日] = [春分, 秋分].map((dd) => {
        ...
        const view = tempo_1.Tempo.at(new tempo_1.CyclicDayTempoRule(dayMsec, day10Zero, stemLength), {
            write_at: dd.write_at,
        });
        return view.slide(stemLength / 2 - view.now_idx - 1);
    });
    const 春 = (0, tempo_1.join)(立春, 夏土用.back());
    const 夏節分 = 立夏.back();
    const 夏 = (0, tempo_1.join)(立夏, 秋土用.back());
    const 秋節分 = 立秋.back();
    const 秋 = (0, tempo_1.join)(立秋, 冬土用.back());
    const 冬節分 = 立冬.back();
    const 冬 = (0, tempo_1.join)(立冬, 春土用.back());
    const 春節分 = 立春2.back();
    const 節分 = 春節分;
    夏土用 = (0, tempo_1.join)(夏土用, 夏節分);
    秋土用 = (0, tempo_1.join)(秋土用, 秋節分);
    冬土用 = (0, tempo_1.join)(冬土用, 冬節分);
    春土用 = (0, tempo_1.join)(春土用, 立春2);
    return {
        立春,
        立夏,
        立秋,
 ...
 */
function 雑節_by_mean(Zz, d, dayMsec, day10Zero, stemLength) {
...
function to_tempo_by_solor(sunny, earthy, geo, dayMsec, dayZero, yearMsec, seasonZero, hourLength, utc, day) {
    ...
    const { 日の出, 日の入 } = solor(sunny, earthy, geo, dayMsec, dayZero, yearMsec, seasonZero, utc, 4, solarNoon);
